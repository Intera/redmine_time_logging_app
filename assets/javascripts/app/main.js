...
// Generated by CoffeeScript 2.3.2
(function() {
  var $$, autocompleteMatchFunc, config, copyDateObject, createIssueSearchDataEntry, createProjectSearchDataEntry, createProjectsIssuesAndSearchData, decimalHoursToColonFormat, decimalHoursToHours, decimalHoursToMinutes, defaultDialogConfig, delim, displayDialog, entityMap, escapeHtml, getBrowserLanguage, ignoreTicketIdRegexp, issueIDToURL, mobileHideAddressBar, onKeypressRejectNaN, padZeros, projectNameToID, removeErrorClass, selectAll, setCursorPosition, setSelectionRange, sortByLocaleIgnoreTicketId, stringContains, stringContainsEvery, timeLimitedFunc, wrapDeferred;
...
(function() {
  ...
  var _isLoaded, activeTimeEntryId, app_config, autocompleteBlur, autocompleteClick, autocompleteFocus, autocompleteSelect, autocompleteSourceDefault, autocompleteSourceRecent, base_init, cache, cacheTimeEntries, cancelEdit, confirmDelete, confirmDialog, createTimeEntriesUrl, dateIsWeekend, datepickerChangeDays, datepickerUpdate, debug, decrementDate, deleteTimeEntry, displayFormDataToDom, displayTimeEntries, editing_init, emptyForm, emptyFormAfterSync, emptyFormButton, exitEditMode, extendBase, fieldNameToDisplayName, fieldNameToSelector, formDataToAPIData, getActivities, getCurrentUser, getDisplayFormData, getFormData, getIssuesProjectsAndSearchData, getSearchFormData, getTimeEntries, hasReceiveFormat, helper, incrementDate, init, initAutoDataReload, initAutocomplete, initDatepicker, initDeleteDialog, insertTimeEntryAsNew, insertTimeEntryRows, loading, missingFieldsError, onSubmit, onTimeEntriesReload, openInRedmineUpdateURL, prevTimeEntry, redmine, reloadSearchData, resetContentTitles, setDate, setTranslation, setTranslationTextAndTitle, startEditMode, sync, timeEntriesLoadingDelayed, timeEntryReceiveToSendFormat, timeEntryToDOM, timeEntryToSearchDataEntry, timeEntryToTableRow, timeFormat, translate, translations, updateTimeEntry, validate, validateFieldExistence, validateOther;
  ...
  missingFieldsError = function(missingFields) {
    return _.map(missingFields, function(ele) {
      if (_.isArray(ele)) {
        return missingFieldsError(ele).join(" " + translate("or") + " ");
      } else {
  ...
  getCurrentUser = function() {
    return redmine.getCurrentUser().done(function(user) {
      var lang;
      cache.user_id = user.user.id;
      lang = user.user.language;
      cache.user_language = lang;
      return window.displayLanguage = translations[lang] ? lang : "en";
    });
  };
  ...
  getIssuesProjectsAndSearchData = function(status) {
    return redmine.getProjectsAndIssues(status, app_config.issueClosedPastDays).done(function(projectsAndIssues) {
      ...
      return redmine.getRecentTimeEntryObjects().done(function(recentTimeEntryObjects) {
        ...
        return cache.searchDataRecent = helper.sortByLocaleIgnoreTicketId(cache.searchDataRecent);
      });
    });
  };

  getActivities = function() {
    return redmine.getActivities().done(function(_activities) {
      var selectEle;
      ...
      return _.each(_activities.activities, function(a) {
        ...
        return cache.activities[a.id] = a.name;
      });
    });
  };
  ...
  resetContentTitles = function() {
    ...
    helper.$$("#comments,#hours,#minutes,#search").each(function() {
      var a;
      a = $(this);
      if (a.val()) {
        return a.removeClass(app_config.titleClass);
      } else {
        a.blur();
        return a.addClass(app_config.titleClass);
      }
    });
    return helper.$$("#activity").change();
  };
  ...
  getSearchFormData = function(str) {
    ...
    if (!(str && _.some(cache.searchData, (function(e) {
      return e.value === str;
    })))) {
  ...
  initAutocomplete = function(projects, issues, searchData, recentlyUpdatedSearchData) {
    return helper.$$("#search").click(autocompleteClick).blur(autocompleteBlur).focus(autocompleteFocus).autocomplete({
  ...
  getFormData = function() {
    ...
    formData = {};
    formData.comments = helper.$$("#comments").hasClass(app_config.titleClass) ? "" : helper.$$("#comments").val();
    if (!helper.$$("#activity").hasClass(app_config.titleClass)) {
  ...
  initDatepicker = function() {
    ...
    return helper.$$("#date").datepicker("setDate", "+0");
  };
  ...
  base_init = function() {
    ...
    loading();
    r = $.when(redmine.init(), getCurrentUser().done(helper.wrapDeferred(initDatepicker)), getActivities(), getIssuesProjectsAndSearchData(app_config.issueStatus)).done(function() {
      helper.$$("h2:first").show();
      helper.$$("#form").show();
      loading(false);
      initAutoDataReload();
      helper.$$("#search").trigger("focus", {
        onlyFocus: true
      }).select();
  ...
  confirmDelete = function() {
    ...
    hours = (helper.$$("#hours").hasClass(app_config.titleClass) ? undefined : helper.$$("#hours").val());
    minutes = (helper.$$("#minutes").hasClass(app_config.titleClass) ? undefined : helper.$$("#minutes").val());
  ...
  exitEditMode = function() {
    ...
    if (helper.$$("#search").hasClass(app_config.titleClass)) {
      helper.$$("#search").trigger("focus", {
        onlyFocus: true
      });
    }
  ...
  timeEntryToDOM = function(timeEntry) {
    ...
    helper.$$("#hours").val((val && (0 !== val) ? val : ""));
    val = helper.decimalHoursToMinutes(timeEntry.hours);
    helper.$$("#minutes").val((val && (0 !== val) ? val : ""));
    ...
    helper.$$("#search").blur();
    resetContentTitles();
    return helper.$$("input[type=text],textarea,select").each(helper.removeErrorClass);
  };
  ...
  init = function() {
    return base_init().done(editing_init);
  };
...
},{"./config":1,"./helper":2,"./redmine":4,"./translations":5}],4:[function(require,module,exports){
// Generated by CoffeeScript 2.3.2
(function() {
  var addCsrfTokenFunc, config, createTimeEntry, defaultRedmineErrorHandler, deleteTimeEntry, getActivities, getCurrentUser, getIssues, getProjects, getProjectsAndIssues, getRecentTimeEntryObjects, getTimeEntries, init, redmineConfig, updateTimeEntry, urls;
