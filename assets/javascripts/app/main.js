...
* http://jqueryui.com
* Includes: core.js, widget.js, mouse.js, position.js, draggable.js, resizable.js, autocomplete.js, button.js, datepicker.js, dialog.js, menu.js, effect.js, effect-highlight.js, effect-slide.js
...
tl = function(key) {
  var ref;
  return ((ref = translations[window.displayLanguage]) != null ? ref[key] : void 0) || key;
};
...
getDisplayLanguage = function() {
  var ref, t;
  if ((ref = App.cache) != null ? ref.user_language : void 0) {
...
App.utility = (function() {
  ...
  decimalHoursToHours = function(arg) {
    return Math.floor(Math.round(arg * 100) / 100);
  };
  decimalHoursToMinutes = function(arg) {
    var res;
  ...
  decimalHoursToColonFormat = function(arg) {
  ...
  issueIDToURL = function(id) {
    return App.config.redmine.urls.issues_redmine + "/" + id;
  };
  missingFieldsError = function(missingFields) {
    return _.map(missingFields, function(ele) {
      if (_.isArray(ele)) {
        return missingFieldsError(ele).join(" " + tl("or") + " ");
      } else {
        $(fieldNameToSelector[ele]).addClass(App.config.errorClass);
        return fieldNameToDisplayName[ele];
      }
    });
  };
  removeErrorClass = function() {
    return $(this).removeClass(App.config.errorClass);
  };
...
App.redmine = (function() {
  var addCsrfTokenFunc, createTimeEntry, defaultRedmineErrorHandler, deleteTimeEntry, getActivities, getCurrentUser, getIssues, getProjects, getProjectsAndIssues, getRecentTimeEntryObjects, getTimeEntries, init, redmineConfig, updateTimeEntry, urls;
  redmineConfig = App.config.redmine;
  urls = redmineConfig.urls;
  addCsrfTokenFunc = function(token) {
    return function(options, origOptions, request) {
  ...
  init = function() {
    return $.ajax({
      ...
      return $.ajaxPrefilter(addCsrfTokenFunc(token));
    });
  };
  jQuery.ajaxSetup({
  ...
  defaultRedmineErrorHandler = function(response, x, y) {
    ...
    if (0 === response.status) {
      ...
      return alert(tl("ticketMessage") + tl("error404NotFound") + (".\nrelevant url: \"" + this.url + "\""));
  ...
  getRecentTimeEntryObjects = function() {
    return $.ajax({
...
App.base = (function() {
  var _isLoaded, autocompleteBlur, autocompleteClick, autocompleteFocus, autocompleteSelect, autocompleteSourceDefault, autocompleteSourceRecent, emptyForm, emptyFormAfterSync, formDataToAPIData, getActivities, getCurrentUser, getFormData, getIssuesProjectsAndSearchData, getSearchFormData, init, initAutoDataReload, initAutocomplete, initDatepicker, loading, onSubmit, openInRedmineUpdateURL, reloadSearchData, resetContentTitles, setTranslation, setTranslationTextAndTitle, sync, validate, validateFieldExistence, validateOther;
  getCurrentUser = function() {
    return App.redmine.getCurrentUser().done(function(user) {
      App.cache.user_id = user.user.id;
      return App.cache.user_language = user.user.language;
    });
  };
  getIssuesProjectsAndSearchData = function(status) {
    return App.redmine.getProjectsAndIssues(status, App.config.issueClosedPastDays).done(function(projectsAndIssues) {
      var t;
      ...
      return App.redmine.getRecentTimeEntryObjects().done(function(recentTimeEntryObjects) {
        App.cache.searchDataRecent = [];
        recentTimeEntryObjects.forEach(function(e, index) {
          if (e.issue_id) {
            e = {
              id: e.issue_id,
            ...
            return App.cache.searchDataRecent.push(App.utility.createIssueSearchDataEntry(e, t[0]));
          } else {
            e = {
              id: e.project_id,
              name: e.project_name,
            ...
            return App.cache.searchDataRecent.push(App.utility.createProjectSearchDataEntry(e));
          }
        });
        return App.cache.searchDataRecent = App.utility.sortByLocaleIgnoreTicketId(App.cache.searchDataRecent);
      });
    });
  };
  getActivities = function() {
    return App.redmine.getActivities().done(function(_activities) {
      var selectEle;
      selectEle = $$("#activity");
      App.cache.activities = {};
      return _.each(_activities.activities, function(a) {
        selectEle.append($("<option>").attr("value", a.id).html(a.name));
        return App.cache.activities[a.id] = a.name;
      });
    });
  };
  sync = function(formData) {
    ...
    if (formData.timeEntryId) {
      return App.redmine.updateTimeEntry(formData.timeEntryId, apiData);
    } else {
      return App.redmine.createTimeEntry(apiData).done(function(timeEntry) {
        var issueOrProject;
        ...
        if (_.isNumber(issueOrProject)) {
          issueOrProject = $("<a>").attr("href", App.utility.issueIDToURL(issueOrProject)).html("#" + issueOrProject);
        }
        return emptyFormAfterSync();
      });
    }
  };
  resetContentTitles = function() {
    $$("#comments,#hours,#minutes,#search").each(function() {
      var a;
      a = $(this);
      if (a.val()) {
        return a.removeClass(App.config.titleClass);
      } else {
        a.blur();
        return a.addClass(App.config.titleClass);
      }
    });
    return $$("#activity").change();
  };
  emptyFormAfterSync = function() {
  ...
  emptyForm = function() {
    ...
    return $$("#activity").val("").change();
  };
  reloadSearchData = function() {
    getIssuesProjectsAndSearchData(App.config.issueStatus);
    return $$(document).trigger("timeEntriesReload");
  };
  getSearchFormData = function(str) {
    var issueID, projectName, project_id;
    if (!str) {
      str = $$("#search").val();
    }
    if (!(str && _.some(App.cache.searchData, (function(e) {
      return e.value === str;
    })))) {
      return false;
    }
  ...
  autocompleteSourceDefault = function(req, handleResponse) {
    return handleResponse(_.filter(App.cache.searchData, App.utility.autocompleteMatchFunc(req.term)));
  };
  autocompleteSourceRecent = function(req, handleResponse) {
    return handleResponse(App.cache.searchDataRecent);
  };
  autocompleteFocus = function(event, options) {
    if (!options || !options.onlyFocus) {
      if (this.value === this.title) {
        ...
        return $$("#search").autocomplete("search", "");
      } else {
        return $$("#search").autocomplete("search", $$("#search").val());
      }
    }
  };
  openInRedmineUpdateURL = function(searchValue) {
    var data, url;
    ...
    if (data) {
      ...
      return $$("button.open-in-redmine").data("href", url).show();
    } else {
      return $$("button.open-in-redmine").hide();
    }
  };
  autocompleteSelect = function(event, ui) {
    return openInRedmineUpdateURL(ui.item.value);
  };
  autocompleteBlur = function() {
  ...
  initAutocomplete = function(projects, issues, searchData, recentlyUpdatedSearchData) {
    return $$("#search").click(autocompleteClick).blur(autocompleteBlur).focus(autocompleteFocus).autocomplete({
      select: autocompleteSelect,
      minLength: App.config.autocompleteMinLength,
      source: function(req, handleResponse) {
        return handleResponse(_.filter(searchData, App.utility.autocompleteMatchFunc(req.term)));
      }
    }).on("keydown", function() {
      $$("#search").autocomplete("option", "minLength", App.config.autocompleteMinLength);
      return $$("#search").autocomplete("option", "source", autocompleteSourceDefault);
  ...
  onSubmit = function() {
    ...
    if (validate(formData)) {
      return sync(formData);
    } else {
      return false;
    }
  };
  getFormData = function() {
    var formData, hours, minutes;
    formData = {};
    formData.comments = $$("#comments").hasClass(App.config.titleClass) ? "" : $$("#comments").val();
    if (!$$("#activity").hasClass(App.config.titleClass)) {
      formData.activity_id = parseInt($$("#activity").val());
    }
  ...
  validateFieldExistence = function(formData) {
    var formDataInclude, formDataIncludeExt, missingFields;
    formDataInclude = function(a) {
      return _.has(formData, a);
    };
    formDataIncludeExt = function(a) {
      if (_.isArray(a)) {
        return _.any(a, formDataInclude);
      } else {
        return formDataInclude(a);
      }
    };
  ...
  validate = function(formData) {
    var results;
    results = [validateFieldExistence, validateOther].map(function(proc) {
      return proc(formData);
    });
  ...
  initDatepicker = function() {
    ...
    return $$("#date").datepicker("setDate", "+0");
  };
  initAutoDataReload = function() {
    var intervalAutoReload, timer;
    if (!App.config.autoReloadInterval) {
      return;
    }
    ...
    timer = void 0;
    return intervalAutoReload(App.config.autoReloadInterval);
  };
  setTranslationTextAndTitle = function(elements, key) {
    var t;
    t = tl(key);
    return elements.attr("title", t).text(t);
  };
  setTranslation = function() {
    ...
    return $("#search,#hours,#minutes,#comments,select").contentTitle();
  };
  init = function() {
    var r;
    App.utility.mobileHideAddressBar();
    loading();
    r = $.when(App.redmine.init(), getCurrentUser().done(App.utility.wrapDeferred(initDatepicker)), getActivities(), getIssuesProjectsAndSearchData(App.config.issueStatus)).done(function() {
      $$("h2:first").show();
      $$("#form").show();
      loading(false);
      initAutoDataReload();
...
App.timeEntryEditing = (function() {
  var cacheTimeEntries, cancelEdit, confirmDelete, createTimeEntriesUrl, dateIsWeekend, datepickerChangeDays, datepickerUpdate, decrementDate, deleteTimeEntry, displayFormDataToDom, displayTimeEntries, exitEditMode, extendBase, getDisplayFormData, getTimeEntries, hasReceiveFormat, incrementDate, init, initDeleteDialog, insertTimeEntryAsNew, insertTimeEntryRows, onTimeEntriesReload, setDate, startEditMode, timeEntriesLoadingDelayed, timeEntryReceiveToSendFormat, timeEntryToDOM, timeEntryToSearchDataEntry, timeEntryToTableRow, timeFormat, updateTimeEntry;
  App.confirmDialog;
  App.timeEntryId;
  App.prevTimeEntry;
  initDeleteDialog = function() {
    ...
    config.buttons[tl("no")] = function() {
      return $(this).dialog("close");
    };
    config.buttons[tl("yes")] = function() {
      $(this).dialog("close");
      return deleteTimeEntry(App.timeEntryId);
    };
    config = _.defaults(config, App.utility.defaultDialogConfig);
    return App.confirmDialog = $("<div>").dialog(config);
  };
  timeFormat = function(hours, minutes) {
    var r;
    r = [];
    if (hours) {
      r.push(hours + " Stunden");
    }
    if (minutes) {
      r.push(minutes + " Minuten");
    }
    return r.join(" ");
  };
  confirmDelete = function() {
    var hours, message, minutes;
    hours = ($$("#hours").hasClass(App.config.titleClass) ? undefined : $$("#hours").val());
    minutes = ($$("#minutes").hasClass(App.config.titleClass) ? undefined : $$("#minutes").val());
    message = timeFormat(hours, minutes) + "<br/><br/>\"" + $$("#search").val() + "\"";
    return App.confirmDialog.html(message).dialog("option", "title", tl("confirmDelete")).dialog("open");
  };
  updateTimeEntry = function() {
    ...
    if (App.base.validate(formData)) {
      return App.base.sync(formData).done(function(response) {
        exitEditMode();
        return $$(document).trigger("timeEntriesReload", "inplace");
      });
    }
  };
  ...
  startEditMode = function(timeEntryId) {
    ...
    $$(".emptyForm").hide();
    return timeEntryToDOM(App.cache.timeEntries[timeEntryId]);
  };
  exitEditMode = function() {
    ...
    if ($$("#search").hasClass(App.config.titleClass)) {
      $$("#search").trigger("focus", {
        onlyFocus: true
      });
    }
    $$(".emptyForm").show();
    return App.prevTimeEntry = false;
  };
  deleteTimeEntry = function(id) {
    return App.redmine.deleteTimeEntry(id).done(function() {
      exitEditMode();
      $$(document).trigger("timeEntriesReload", "inplace");
      return App.base.emptyForm();
    });
  };
  ...
  timeEntryToTableRow = function(a, even) {
    ...
    if (isNaN(spent_hours)) {
      ...
      spent_hours = spent_hours >= 1 ? Math.round(spent_hours) : App.utility.decimalHoursToColonFormat(spent_hours);
    }
    estimated_hours = ((ref1 = a.issue) != null ? ref1.estimated_hours : void 0) || a.project.estimated_hours;
    if (isNaN(estimated_hours)) {
      estimated_hours = 0;
    } else {
      estimated_hours = estimated_hours >= 1 ? Math.round(estimated_hours) : App.utility.decimalHoursToColonFormat(estimated_hours);
    }
    estimates = spent_hours + (estimated_hours ? "/" + estimated_hours : "");
    return App.cache.template.timeEntry({
      entry_id: a.id,
      time: time,
      estimates: estimates,
      name: name,
      comments: (a.comments ? App.utility.escapeHtml(a.comments) : ""),
      activity: App.cache.activities[a.activity.id],
      projectOrIssueUrl: projectOrIssueUrl,
      timeEntriesUrl: createTimeEntriesUrl(a),
      classes: (even ? "even" : "odd") + (classes ? " " + classes : "")
    });
  };
  insertTimeEntryRows = function(timeEntries) {
    ...
    return _.each(timeEntries, function(a, index) {
      var even, val;
      even = (index % 2) === 0;
      if (a.issue && !App.cache.issues[a.issue.id]) {
  ...
  displayTimeEntries = function(timeEntries, config) {
    ...
    if (timeEntries.length) {
      daySpentTime = _.foldl(timeEntries, function(prev, a) {
        return prev + a.hours;
      }, 0);
  ...
  getTimeEntries = function(data, config, noEntriesFound) {
    ...
    return App.redmine.getTimeEntries(data).done(function(response) {
      var timeEntries;
      ...
      if (response.time_entries && (response.time_entries.length === 0)) {
        if (!noEntriesFound || !noEntriesFound()) {
          return $$("#loading .text").html(tl("noTimeEntriesLoaded"));
        }
      } else {
        return App.base.loading(false);
      }
    });
  };
  timeEntryToSearchDataEntry = function(a) {
    if (a.issue) {
      ...
      return App.utility.createIssueSearchDataEntry(a.issue, App.cache.projects);
    } else {
      if (a.project) {
        return App.utility.createProjectSearchDataEntry(App.cache.projects[a.project.id]);
      }
    }
  };
  ...
  timeEntryReceiveToSendFormat = function(a) {
    var receiveFormat;
    if (App.debug && !hasReceiveFormat(a)) {
  ...
  extendBase = function() {
    ...
    return App.base.onSubmit = function() {
      var r;
      r = baseOnSubmit();
      if (r) {
  ...
  datepickerChangeDays = function(element, difference) {
    ...
    if ((prevDate.getFullYear() !== newDate.getFullYear()) || (prevDate.getMonth() !== newDate.getMonth()) || (prevDate.getDate() !== newDate.getDate())) {
      return datepickerUpdate(function() {
        if (dateIsWeekend(nextDate)) {
          return datepickerChangeDays($$("#date"), Math.min(difference, 1));
        }
      });
    }
  };
  incrementDate = function() {
    return datepickerChangeDays($$("#date"), 1);
  };
...
App.base.init().done(App.timeEntryEditing.init);
