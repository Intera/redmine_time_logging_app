#depends on: DOM.ready, redmine.coffee
App.base = (->
  getCurrentUser = ->
  ...
  getIssuesProjectsAndSearchData = (status) ->
    #required global variables: searchData, projects, issues (now in App.cache)
  ...
  getActivities = ->
    App.redmine.getActivities().done (_activities) ->
      ...
      _.each _activities.activities, (ele) ->
        selectEle.append $("<option>").attr("value", ele.id).html(ele.name)
        App.cache.activities[ele.id] = ele.name
  ...
  resetContentTitles = ->
    ...
    $$("#comments,#hours,#minutes,#search").each ->
      ele = $(this)
      if ele.val()
        ele.removeClass App.config.titleClass
      else
        ele.blur()
        ele.addClass App.config.titleClass
    $$("#activity").change()
  ...
  getSearchFormData = (str) ->
    ...
    if issueID
      ...
      #Prüfung, ob es sich um Ober- oder Unterprojekt handelt
  ...
  formDataToAPIData = (formData) ->
    #direct mapping
    res = _.pick(formData, "activity_id", "comments", "hours", "project_id", "issue_id")
    #format changing
    res.spent_on = $.datepicker.formatDate("yy-mm-dd", formData.date)
    # convert hours and minutes to decimal hours
    res.hours = (res.hours or 0) + ((formData.minutes or 0) / 60)
    res
  ...
  validateFieldExistence = (formData) ->
    formDataInclude = (ele) ->
      _.has formData, ele
    formDataIncludeExt = (ele) ->
      if _.isArray(ele)
        _.any ele, formDataInclude
      else
        formDataInclude ele
  ...
  setTranslation = ->
    ...
    # title attribute will be set by .contentTitle
  ...
  init = ->
    ...
    loading()
    res = $.when(App.redmine.init(), getCurrentUser().done(App.utility.wrapDeferred(initDatepicker)), getActivities(), getIssuesProjectsAndSearchData(App.config.issueStatus)).done(->
      $$("h2:first").show()
      $$("#form").show()
      loading(false)
      initAutoDataReload()
    ...
    $(".submit").click ->
      #must be passed this way, otherwise onSubmit is passed by value and cannot be extended
      App.base.onSubmit()
